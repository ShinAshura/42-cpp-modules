...
// for ScavTrap contructor
ClapTrap::ClapTrap(std::string name, int hp, int ep, int ad) : _name(name), _hit_point(hp), _energy_point(ep), _attack_damage(ad)
{
	std::cout << "ClapTrap constructor called for " << this->_name << std::endl;
}
...
// *this = op; // incorrect and dangerous ? not to be used ?
